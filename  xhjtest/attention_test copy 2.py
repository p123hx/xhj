...
from attention_const import  HIDDEN_SIZE, DECODER_LAYERS, SRC_VOCAB_SIZE, TRG_VOCAB_SIZE, SHARE_EMB_AND_SOFTMAX, \
    SOS_ID, EOS_ID
...
def main(test_en_text, checkpoint_path, src_vocab_file, trg_vocab_file):
    ...
    print(datetime.now(), test_en_text)
    ...
    print(datetime.now(), test_en_ids)
    ...
    # 输出翻译结果。
    print(datetime.now(), output_text.encode('utf8').decode(sys.stdout.encoding))
    sess.close()

def translate_test():
    checkpoint_path = "./model/attention_ckpt-100"

    test_en_text = "This is a test . <eos>"
    test_en_text = "To improve is to change . <eos>"
    test_en_text = "To be or not to be , that is the question . <eos>"

    src_vocab_file = "./en.vocab"
    trg_vocab_file = "./zh.vocab"

    main(test_en_text, checkpoint_path, src_vocab_file, trg_vocab_file)
