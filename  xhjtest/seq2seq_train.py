...
from attention_const import HIDDEN_SIZE, SRC_VOCAB_SIZE, TRG_VOCAB_SIZE, BATCH_SIZE, NUM_EPOCH, \
    KEEP_PROB, MAX_GRAD_NORM, SHARE_EMB_AND_SOFTMAX, MAX_LEN, SOS_ID, NUM_LAYERS
...
def run_epoch(session, cost_op, train_op, saver, step, checkpoint_path):
    # 训练一个epoch。
    ...
    while True:
        try:
            ...
            if step % 10 == 0:
                print(time.time(), "After %d steps, per token cost is %.3f" % (step, cost), flush=True)
            # 每200步保存一个checkpoint。
            if step % 200 == 0:
...
def main(src_train_data, trg_train_data, checkpoint_path):
    ...
    with tf.Session() as sess:
        ...
        for i in range(NUM_EPOCH):
            print(time.time(), "In iteration: %d" % (i + 1), flush=True)
...
if __name__ == "__main__":
    ...
    checkpoint_path = "./model/xhj_seq2seq_ckpt"  # checkpoint保存路径。
