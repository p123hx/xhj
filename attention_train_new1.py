...
from attention_const import HIDDEN_SIZE, DECODER_LAYERS, SRC_VOCAB_SIZE, TRG_VOCAB_SIZE, BATCH_SIZE, NUM_EPOCH, \
    KEEP_PROB, MAX_GRAD_NORM, SHARE_EMB_AND_SOFTMAX, MAX_LEN, SOS_ID
...
# SRC_VOCAB_SIZE = 62600                 # 源语言词汇表大小。
# TRG_VOCAB_SIZE = 55500                 # 目标语言词汇表大小。
...
def run_epoch(session, cost_op, train_op, saver, step, checkpoint_path="./attention_ckpt"):
    # 训练一个epoch。
    ...
    while True:
        try:
            ...
            learning_rate = tf.train.polynomial_decay(learning_rate=0.01, global_step=global_step, decay_steps=50000, end_learning_rate=0.00001, power=1.0)
            learning_r = session.run(learning_rate, feed_dict={global_step: step})
            cost, _ = session.run([cost_op, train_op], feed_dict={learning_rate: learning_r})
            if step % 10 == 0:
                print(datetime.now(), "After %d steps, per token cost is %.3f" % (step, cost), flush=True)
            # 每200步保存一个checkpoint。
            if step % 200 == 0:
...
def main(src_train_data, trg_train_data,restore_path, checkpoint_path):
    # 定义初始化函数。
    ...
    learning_rate = tf.Variable(tf.constant(1))
    ...
    with tf.Session() as sess:
        ...
        for i in range(NUM_EPOCH):
            print(datetime.now(), "In iteration: %d" % (i + 1), flush=True)
            sess.run(iterator.initializer)
            step = run_epoch(sess, cost_op, train_op, saver, step, checkpoint_path)
...
if __name__ == "__main__":
    # src_train_data = "./train.en"  # 源语言输入文件。
    # trg_train_data = "./train.zh"  # 目标语言输入文件。
    # checkpoint_path = "./model/attention_ckpt"  # checkpoint保存路径。

    src_train_data = "./data/xhj_q_train"  # 源语言输入文件。
    trg_train_data = "./data/xhj_a_train"  # 目标语言输入文件。
    restore_path= "./model"
    checkpoint_path = "./model/xhj_attention_ckpt"  # checkpoint保存路径。

    main(src_train_data, trg_train_data, restore_path,checkpoint_path)
